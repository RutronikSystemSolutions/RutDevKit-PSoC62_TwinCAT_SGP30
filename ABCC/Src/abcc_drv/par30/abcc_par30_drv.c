...
void ABCC_DrvPar30RunDriverTx( void )
{
   if(eState == SM_RDY_TO_PING )
   {
      ...
      */
      ...
      /*
      ** Change state before writing to control register to avoid
      ** race condition in case of interrupt driven configuration.
      */
...
{
   /*
   ** The ABCC30-style PD areas are only 256 bytes. Pull the plug if someone
   ** orders a larger PD size than that.
   */
   ABCC_ASSERT_ERR( iReadPdSize <= 256, ABCC_SEV_FATAL, ABCC_EC_RDPD_SIZE_ERR, (UINT32)iReadPdSize );
   ABCC_ASSERT_ERR( iWritePdSize <= 256, ABCC_SEV_FATAL, ABCC_EC_WRPD_SIZE_ERR, (UINT32)iWritePdSize );
