...
static ABCC_CmdSeqCmdStatusType FirmwareVersionCmd(ABP_MsgType* psMsg);
static ABCC_CmdSeqRespStatusType FirmwareVersionResp(ABP_MsgType* psMsg);
...
static ABCC_CmdSeqRespStatusType FirmwareVersionResp( ABP_MsgType* psMsg )
{
   ...
   ABCC_ASSERT_ERR( ABCC_VerifyMessage( psMsg ) == ABCC_EC_NO_ERROR,
                    ABCC_SEV_WARNING, ABCC_EC_RESP_MSG_E_BIT_SET,
                    (UINT32)ABCC_GetErrorCode( psMsg ) );
   ABCC_GetMsgData8( psMsg, &abcc_bFWMajor, 0 );
   ABCC_GetMsgData8( psMsg, &abcc_bFWMinor, 1 );
   ABCC_GetMsgData8( psMsg, &abcc_bFWBuild, 2 );
   DEBUG_EVENT( ( "RSP MSG_GET_FW_VERSION: %d.%d.%d\n", abcc_bFWMajor, abcc_bFWMinor, abcc_bFWBuild ) );
   return( ABCC_EXEC_NEXT_COMMAND );
}
